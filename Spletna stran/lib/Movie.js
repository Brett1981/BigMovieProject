...
var div     = "<div>"   , span  = "<span>"  , 
    ...
    a       = "<a>"     , p     = "<p>"     , i     = "<i>",
    li      = "<li>"    , ul    = "<ul>"    , 
    table   = "<table>" , tbody = "<tbody>" , thead = "<thead>" , td    = "<td>"    , tr    = "<tr>", th    = "<th>"; 
    ...
    this.createModalViewDiv = function(data){
        ...
        top =  $(div).addClass("top-modal-data").append(
                    ...
                    $(div).addClass("right-modal-data").append(
                        $(table).addClass("table-modal").append(
                            $(thead).append(
                                $(th).attr({width:"48%"}),
                                $(th).attr({width:"48%"})
                            ),
                            $(tbody).append(
                                $(tr).append(
                                    $(td).attr({colspan:"2",align:"left"}).append(
                                        $(p).text("Tagline: "+minfo.tagline)
                                    )
                                ),
                                $(tr).append(
                                    $(td).attr({colspan:"2",align:"left"}).append(
                                ...
                                $(tr).append(
                                    $(td).attr({colspan:"2"}).append(
                                        $(a).attr({href:minfo.homepage,style:"display:-webkit-inline-box;padding:5px;margin:auto;box-sizing:border-box;width:100%;"}).append(
                                            $(i).addClass("material-icons").attr("style","font-size:48px;color:darkviolet").text("link"),
                                ...
                                $(tr).append(
                                    $(td).attr({colspan:"2"}).append(
                                        $(a).attr({href:'http://www.imdb.com/title/'+minfo.imdb_id, style:"display:-webkit-inline-box;padding:5px;margin:auto;box-sizing: border-box;width:100%;"} ).append(
                                            $(i).addClass("material-icons").attr("style","font-size:48px;color:darkviolet").text("movie"),
                                            $(p).text("iMDB")
                                        )
                                    )
                                )
